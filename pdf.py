...
class RAGPipeline:
    def __init__(self, api_key: str = None, model_name: str = 'all-MiniLM-L6-v2', embedding_dim: int = 384):
        ...
        Args:
            api_key (str): Hugging Face API key. If None, will try to get from environment variable
        ...
        if not self.api_key:
            st.error("Hugging Face API key not found. Please provide it in the sidebar.")
            return
        ...
        try:
            ...
            st.error(f"Error initializing Hugging Face model: {str(e)}")
            return
    ...
    def search_documents(self, query: str, k: int = 5) -> str:
        ...
        if not indices.any():  # Fixed FAISS query validation
